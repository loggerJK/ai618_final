...
def interpolate_fisher_rao(A: torch.Tensor, identity_scale: float, eps=1e-12):
    """
    Fisher–Rao / Hellinger geodesic between I and A.

    Parameters
    ----------
    A : (..., N, N)  attention map (row sums = 1)
    identity_scale : float in [0,1]
        1 → identity, 0 → A
    """
    t = 1.0 - identity_scale
    print(f"t: {t}")
    if t <= 0:
        N = A.size(-1)
        eye = torch.eye(N, dtype=A.dtype, device=A.device)
        return eye.expand(A.shape[:-2] + (N, N))
    if t >= 1:
        return A.clone()

    sqrtA = torch.sqrt(A.clamp_min(0.0))
    N = A.size(-1)
    eye = torch.eye(N, dtype=A.dtype, device=A.device)
    eye = eye.expand_as(A)
    
    S = (1 - t) * eye + t * sqrtA              # numerator before squaring
    out = S.pow(2)
    out = out / (out.sum(-1, keepdim=True) + eps)  # renormalise
    if out.isnan().any() or (out > 1).any():
        print(f"out is nan")
        breakpoint()
    return out

# Efficient implementation equivalent to the following:
def skip_attention(query, key, value, attn_mask=None, dropout_p=0.0,
        is_causal=False, scale=None, enable_gqa=False, skip_heads=None, identity_scale=0.0, save_attn_weight=False, perturb_type=None, perturb_heads=None, is_flux=False) -> torch.Tensor:
    # raise NotImplementedError("skip_attention is not implemented")
    ...
    if is_flux: # [text, image]
        # img_size = 4096
        txt_size = 512
    else: # [image, text] for sd3-medium 
        # img_size = 4096
        txt_size = 333
    # assert img_size + txt_size == L
    img_size = L - txt_size
    # print(f"img_size: {img_size}, txt_size: {txt_size}")
    
    if "[LOGIT_PERTURB]" in perturb_type:
        if is_flux:
            raise NotImplementedError("Flux attention is not implemented")
        if "II_identity" in perturb_type and "renormalize" not in perturb_type:
            attn_bias[:img_size, :img_size] = float("-inf")
            attn_bias[:img_size, :img_size].fill_diagonal_(0)
        elif "II_mask" in perturb_type:
            attn_bias[:img_size, :img_size] = float("-inf")
        elif "IT_mask" in perturb_type:
            attn_bias[:img_size, img_size:] = float("-inf")
        elif "TI_mask" in perturb_type:
            attn_bias[img_size:, :img_size] = float("-inf")
        elif "TT_mask" in perturb_type:
            attn_bias[img_size:, img_size:] = float("-inf")
        elif "TT_identity" in perturb_type:
            attn_bias[img_size:, img_size:] = float("-inf")
            attn_bias[img_size:, img_size:].fill_diagonal_(0)
        else:
    ...
    if "temp_control" in perturb_type:
        temperature = float(perturb_type.split("@temperature=")[-1].split("@")[0])
        if temperature >= 1e5:
            # Argmax-based one-hot attention (T → ∞ case)
            max_idx = attn_weight.argmax(dim=-1, keepdim=True)  # [B, h, q, 1]
            perturbed_attn_weight = torch.zeros_like(attn_weight)
            perturbed_attn_weight.scatter_(-1, max_idx, 1.0)
        else:
            perturbed_attn_weight = temperature * (query @ key.transpose(-2, -1)) * scale_factor
            perturbed_attn_weight = torch.softmax(perturbed_attn_weight, dim=-1)
    
    if "softmax_t_annealing" in perturb_type:
        scale = float(perturb_type.split("@scale=")[-1].split("@")[0])
        B, H, _, _ = query.shape
        identity_matrix = torch.eye(L, S, dtype=query.dtype, device=query.device).repeat(B, H, 1, 1)
        M = attn_weight.max(dim=-1, keepdim=False)
        perturbed_attn_weight = (1 - scale) * identity_matrix * M + scale * (query @ key.transpose(-2, -1)) * scale_factor
        perturbed_attn_weight = torch.softmax(perturbed_attn_weight, dim=-1)
    
    if "[LOGIT_PERTURB]" in perturb_type:
        perturbed_attn_weight = attn_weight + attn_bias
        perturbed_attn_weight = torch.softmax(perturbed_attn_weight, dim=-1)
    elif "smoothed_energy" in perturb_type:
        if "all" in perturb_type:
            # apply mean across query dim
            perturbed_query = query.mean(dim=-2, keepdim=True)
            perturbed_query = perturbed_query.repeat(1, 1, L, 1)
        elif "img" in perturb_type:
            if is_flux:
                raise NotImplementedError("Flux attention is not implemented")
            query_img_mean = query[:, :, :img_size, :].mean(dim=-2, keepdim=True)
            query_img_mean = query_img_mean.repeat(1, 1, img_size, 1)
            perturbed_query = query.clone()
            perturbed_query[:, :, :img_size, :] = query_img_mean
        elif "txt" in perturb_type:
            if is_flux:
                raise NotImplementedError("Flux attention is not implemented")
            query_txt_mean = query[:, :, img_size:, :].mean(dim=-2, keepdim=True)
            query_txt_mean = query_txt_mean.repeat(1, 1, L - img_size, 1)
            perturbed_query = query.clone()
            perturbed_query[:, :, img_size:, :] = query_txt_mean
        else:
            raise ValueError(f"Invalid perturbation type: {perturb_type}")
        perturbed_attn_weight = perturbed_query @ key.transpose(-2, -1) * scale_factor
        perturbed_attn_weight = torch.softmax(perturbed_attn_weight, dim=-1)
    else:
        attn_weight = attn_weight + attn_bias
        
    attn_weight = torch.softmax(attn_weight, dim=-1)
    B, H, L, S = attn_weight.shape

    img_size = 4096
    txt_size = 333
    if perturb_type is None:
        raise ValueError(f"perturb_type is None")
    if "[PROB_PERTURB]" in perturb_type:
        if "attention_identity" in perturb_type: # Identity Attention
            perturbed_attn_weight = torch.eye(L, S, dtype=query.dtype, device=query.device).repeat(B, H, 1, 1)
        elif "II_identity_IT_mask" in perturb_type:
            if is_flux:
                raise NotImplementedError("Flux attention is not implemented")
            perturbed_attn_weight = torch.eye(L, S, dtype=query.dtype, device=query.device).repeat(B, H, 1, 1)
            attn_weight[:, :, :img_size, :] = perturbed_attn_weight[:, :, :img_size, :]
        elif "uniform" in perturb_type:
            perturbed_attn_weight = torch.ones_like(attn_weight) / S
        elif "zeroout" in perturb_type:
            # make off-diagonal elements zero
            zeroout_mask = torch.ones_like(attn_weight)
            if "II_identity" in perturb_type:
                if is_flux:
                    raise NotImplementedError("Flux attention is not implemented")
                diag_mask = torch.eye(L, S, dtype=query.dtype, device=query.device).repeat(B, H, 1, 1)
                zeroout_mask[:, :, :img_size, :img_size] = diag_mask[:, :, :img_size, :img_size]
            elif "II_mask" in perturb_type:
                if is_flux:
                    raise NotImplementedError("Flux attention is not implemented")
                zeroout_mask[:, :, :img_size, :img_size] = 0
            elif "IT_mask" in perturb_type:
                if is_flux:
                    raise NotImplementedError("Flux attention is not implemented")
                zeroout_mask[:, :, :img_size, img_size:] = 0
                ...
                raise ValueError(f"Invalid perturbation type: {perturb_type}")
            perturbed_attn_weight = attn_weight * zeroout_mask
        elif "II_identity_renormalize" in perturb_type:
            if is_flux:
                raise NotImplementedError("Flux attention is not implemented")
            # sum up the image-image attention and move the mass to the diagonal
            row_sums = attn_weight[:, :, :img_size, :img_size].sum(dim=-1) # [1, 24, 4096]
            new_img_img_attn = torch.zeros_like(attn_weight)
            new_img_img_attn[:, :, :img_size, :img_size] = torch.diag_embed(row_sums)
            perturbed_attn_weight = attn_weight.clone()
            perturbed_attn_weight[:, :, :img_size, :img_size] = new_img_img_attn[:, :, :img_size, :img_size]
        else:
            raise ValueError(f"Invalid perturbation type: {perturb_type}")
        
    # Apply perturbation to specific heads
    if perturb_heads is not None:
        not_perturb_head_indices = [i for i in range(H) if i not in perturb_heads]
        perturbed_attn_weight[:, not_perturb_head_indices, :, :] = attn_weight[:, not_perturb_head_indices, :, :]
    
    # Perform interpolation
    if "fisher_rao" in perturb_type: # Fisher-Rao interpolation
        attn_weight = interpolate_fisher_rao(attn_weight, identity_scale)
    elif "scale" in perturb_type: # linear interpolation
        identity_scale = float(perturb_type.split("@scale=")[-1].split("@")[0]) if "@scale=" in perturb_type else 0.0
        attn_weight = (1 - identity_scale) * attn_weight + identity_scale * perturbed_attn_weight
    else:
        attn_weight = perturbed_attn_weight
    
    # Apply dropout
    attn_weight = torch.dropout(attn_weight, dropout_p, train=True)
    attn_weight = attn_weight.to(dtype=query.dtype)
    
    feature = attn_weight @ value

    if save_attn_weight:
        return feature, attn_weight, value
    else:
        return feature
...
class JointAttnProcessor2_0:
    ...
    def __call__(
        ...
        perturb_heads: Optional[List[Tuple[int, int]]] = None,
        *args,
        **kwargs,
    ) -> torch.FloatTensor:
        residual = hidden_states # [1, 4096, 1536]

        batch_size = hidden_states.shape[0]

        # `sample` projections.
        query = attn.to_q(hidden_states) # [1, 4096, 1536]
        key = attn.to_k(hidden_states) # [1, 4096, 1536]
        value = attn.to_v(hidden_states) # [1, 4096, 1536]

        inner_dim = key.shape[-1] # 1536
        head_dim = inner_dim // attn.heads # 1536 // 24 = 64

        query = query.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2) # [1, 4096, 1536] -> [1, 4096, 24, 64] -> [1, 24, 4096, 64]
        key = key.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2) # [1, 4096, 1536] -> [1, 4096, 24, 64] -> [1, 24, 4096, 64]
        value = value.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2) # [1, 4096, 1536] -> [1, 4096, 24, 64] -> [1, 24, 4096, 64]
        ...
        if encoder_hidden_states is not None:
            encoder_hidden_states_query_proj = attn.add_q_proj(encoder_hidden_states) # [1, 333, 1536]
            encoder_hidden_states_key_proj = attn.add_k_proj(encoder_hidden_states) # [1, 333, 1536]
            encoder_hidden_states_value_proj = attn.add_v_proj(encoder_hidden_states) # [1, 333, 1536]

            encoder_hidden_states_query_proj = encoder_hidden_states_query_proj.view(
                batch_size, -1, attn.heads, head_dim
            ).transpose(1, 2) # [1, 333, 1536] -> [1, 333, 24, 64] -> [1, 24, 333, 64]
            ...
            query = torch.cat([query, encoder_hidden_states_query_proj], dim=2) # [1, 24, 4096, 64] -> [1, 24, 4096 + 333, 64]
            key = torch.cat([key, encoder_hidden_states_key_proj], dim=2) # [1, 24, 4096, 64] -> [1, 24, 4096 + 333, 64]
            value = torch.cat([value, encoder_hidden_states_value_proj], dim=2) # [1, 24, 4096, 64] -> [1, 24, 4096 + 333, 64]

        if perturb_type is None:
            hidden_states = F.scaled_dot_product_attention(query, key, value, dropout_p=0.0, is_causal=False) 
        else:
            hidden_states = skip_attention(query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False, perturb_type=perturb_type, perturb_heads=perturb_heads)
...
class SaveJointAttnProcessor2_0:
    """Attention processor used typically in processing the SD3-like self-attention projections."""
    ...
    def __call__(
        ...
        **kwargs,
        ...
        raise NotImplementedError("SaveJointAttnProcessor2_0 is not implemented")
        residual = hidden_states # [1, 4096, 1536]
        ...
        if perturb_type is None:
            hidden_states, attn_weight, value = skip_attention(query, key, value, dropout_p=0.0, is_causal=False, save_attn_weight=True) # 
        else:
            if "TT_mask" in perturb_type \
                or "TT_identity" in perturb_type \
                or "TI_mask" in perturb_type \
                or "IT_mask" in perturb_type \
                or "II_mask" in perturb_type \
                or "II_identity" in perturb_type \
                or "attention_identity" in perturb_type \
                or "attention_smooth_identity" in perturb_type:
                print(f"attention_mask: {perturb_type}")
                # Apply perturbation
                img_size = 4096
                txt_size = 333

                # create a full mask with all entries set to 0
                batch_size, num_heads, seq_len, head_dim = query.size() # q: [1, 24, 512 + 4096, 128]
                head_mask = torch.zeros((seq_len, seq_len), device=query.device, dtype=query.dtype)

                # if "attention_identity" in perturb_type:
                #     print("attention_identity")
                #     head_mask[:, :] = float("-inf")
                #     head_mask[:, :].fill_diagonal_(0)
                # if "II_mask" in perturb_type:
                #     print("II_mask")
                #     head_mask[:img_size, :img_size] = float("-inf")
                # if "II_identity" in perturb_type:
                #     print("II_identity")
                #     head_mask[:img_size, :img_size] = float("-inf")
                #     head_mask[:img_size, :img_size].fill_diagonal_(0)
                # if "IT_mask" in perturb_type:
                #     print("IT_mask")
                #     head_mask[:img_size, img_size:] = float("-inf")
                # if "TI_mask" in perturb_type:
                #     print("TI_mask")
                #     head_mask[img_size:, :img_size] = float("-inf")
                # if "TT_mask" in perturb_type:
                #     print("TT_mask")
                #     head_mask[img_size:, img_size:] = float("-inf")
                # if "TT_identity" in perturb_type:
                #     print("TT_identity")
                ...
                # if "head" in perturb_type:
                #     perturb_head = int(perturb_type.split("head_")[-1])
                #     print(f"perturb_head: {perturb_head}")
                #     full_mask = torch.zeros_like(head_mask).unsqueeze(0).unsqueeze(0)  # Add batch and num_heads dimensions [1, 1, 512 + 4096, 512 + 4096]
                #     full_mask = full_mask.repeat(batch_size, num_heads, 1, 1)
                #     full_mask[:, perturb_head, :, :] = head_mask
                # else:
                #     head_mask = head_mask.unsqueeze(0).unsqueeze(0)  # Add batch and num_heads dimensions [1, 1, 512 + 4096, 512 + 4096]
                #     full_mask = head_mask # Apply the same mask to all heads
                # hidden_states = F.scaled_dot_product_attention(query, key, value, attn_mask=full_mask, dropout_p=0.0, is_causal=False)
                if "attention_identity" in perturb_type:
                    print("attention_identity")
                    head_idx = int(perturb_type.split("attention_identity_head_")[-1])
                    hidden_states, attn_weight, value = skip_attention(query, key, value, dropout_p=0.0, is_causal=False, identity=True, skip_heads=head_idx, identity_scale=1.0, save_attn_weight=True)
                elif "attention_smooth_identity" in perturb_type:
                    smoothness = float(perturb_type.split("attention_smooth_identity@")[-1])
                    hidden_states, attn_weight, value = skip_attention(query, key, value, dropout_p=0.0, is_causal=False, identity=True, skip_heads=None, identity_scale=smoothness, save_attn_weight=True)
                else:
                    hidden_states, attn_weight, value = skip_attention(query, key, value, dropout_p=0.0, is_causal=False, save_attn_weight=True)
        ...
        if encoder_hidden_states is not None:
            return hidden_states, encoder_hidden_states, attn_weight, value
        else:
            return hidden_states, attn_weight, value
...
class FluxAttnProcessor2_0:
    ...
    def __call__(
        ...
        if perturb_type is None:
            ...
            hidden_states = skip_attention(
                query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False, perturb_type=perturb_type, perturb_heads=perturb_heads, is_flux=True
            )
